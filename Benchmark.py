...
class Benchmark:
    ...
    def __init__(self, graph: Graph, algos: List[Algorithm], stations_traversed: List[Station], repetitions=0):
        self.graph = graph
        self.algos = algos
    ...
    def do_bench(self):
        ...
        # if input indicates execution time KPI, execute algorithms using the bench for many repetitions
                ...
                for kpi in [ExecutionTime(), CpuTime()]:
                    ...
                    if counter == 1:
                        for i in range(case['num']):
                            exec_time[algo.get_name()].append(test.find_measurement())
                    elif counter == 2:
                        for i in range(case['num']):
                            cpu_time[algo.get_name()].append(test.find_measurement())
                    counter += 1
            ...
            for algo in self.algos:
                for case in bench:
                    ...
                    for kpi in [Compares(), Inserts(), Visited()]:
                        test = Test(self.graph, algo, kpi, case['from_node_id'], case['to_node_id'])
                        val = {'traversed': case['num'], 'value': test.find_measurement()}
                        if counter == 1:
    ...
    # repetitions > 0 indicates execution time or CPU time KPI (since repeatedly executing algorithms is needed)
    def get_bench(self):
        bench = []
        done = {}
        counter = 0
        ...
        for i in random_nodes:
            for j in random_nodes:
                ...
                # if input arguments indicate an execution time KPI, return a single bench
                if self.repetitions > 0 and paths.find_shortest_path()['stations traversed'] == num:
                    ...
                    return bench
                # if input arguments indicate an interative KPI, return multiple benches
